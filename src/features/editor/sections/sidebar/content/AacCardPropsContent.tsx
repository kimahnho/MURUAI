/**
 * AAC 카드(v2) 복합 요소 선택 시 사이드바에 표시되는 속성 편집 패널.
 * "AAC" 탭(이미지 검색/삽입)과 "박스" 탭(라벨 위치/테두리/배경 스타일)을 제공한다.
 */
...
const AacCardPropsContent = () => {
  ...
  const isAacCardV2 = panelData?.type === "aacCardV2";

  // 선택 해제 시 사이드바를 템플릿 탭으로 복원한다.
  useEffect(() => {
    if (!panelData || panelData.type !== "aacCardV2") {
  ...
  const data = panelData as AacCardV2PanelData;
  const { element } = data;
  ...
  const boxContent = (
    <div className="flex flex-col w-full gap-4">
      ...
      {/* 박스 색상 — 이미지가 있을 때는 backgroundColor로, 없을 때는 fill로 저장한다 */}
      <div className="flex flex-col gap-2">
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 min-h-0 overflow-y-auto scrollbar-hide">
        {activeTab === "aac" ? <AACContent /> : boxContent}
      </div>
    </div>
  );
};

export default AacCardPropsContent;
