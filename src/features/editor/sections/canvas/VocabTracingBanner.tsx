 ...
 * 클릭 시 크기 선택 모달을 열고, 확정 시 따라쓰기 페이지를 생성한다.
 */
...
import {
  extractVocabData,
  getVocabUnfilledReason,
} from "@/features/editor/utils/tracingGridUtils";

import VocabTracingSizeModal from "./VocabTracingSizeModal";
...
const VocabTracingBanner = ({
  ...
  const isModalOpen = useVocabTracingStore((s) => s.isModalOpen);
  const cellSize = useVocabTracingStore((s) => s.cellSize);
  const setCellSize = useVocabTracingStore((s) => s.setCellSize);
  const openModal = useVocabTracingStore((s) => s.openModal);
  const closeModal = useVocabTracingStore((s) => s.closeModal);
  const confirmAndGenerate = useVocabTracingStore((s) => s.confirmAndGenerate);
  ...
  const vocabData = isFilled ? extractVocabData(selectedPage.elements) : [];
  const maxCharCount = vocabData.reduce(
    (max, item) => Math.max(max, [...item.label].length),
    0,
  );

  const handleClick = () => {
    openModal(selectedPageId);
  };
