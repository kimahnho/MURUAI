...
const buildMindMap = (config: MindMapConfig, x: number, y: number, cw: number, orientation?: Orientation): { elements: CanvasElement[]; height: number } => {
  ...
  // mindMapLayout.ts의 비율 좌표 기준 영역을 페이지에 맞게 균일 스케일링.
  // 마인드맵 레이아웃은 가로/세로에 따라 다른 콘텐츠 치수(180×267 또는 267×180 mm)를 쓰므로
  // mindMapLayout 쪽이 그대로 계산한 비율 좌표를 여기서 동일하게 스케일링만 맞춰 배치한다.
  const LAYOUT_W_MM = orientation === "horizontal" ? 267 : 180;
  const LAYOUT_H_MM = orientation === "horizontal" ? 180 : 267;
  const LAYOUT_W_PX = mmToPx(LAYOUT_W_MM);
  const LAYOUT_H_PX = mmToPx(LAYOUT_H_MM);
  const availH = getPageH(orientation) - MARGIN * 2;

  // 비율 유지하며 페이지에 맞는 스케일 계산 (겹침 방지). 1.0 캡으로 기본 디자인 크기 유지
  ...
  // Y축 offset은 적용하지 않음 — 노드 비율 좌표(center=0.5)가 areaH 내 자체 중앙 배치를 보장
  const offsetX = (cw - areaW) / 2;

  // 동적 노드 크기 — 1차별 개별 2차 수(level2_counts)가 있으면 그것도 반영
...
export const buildWorksheetComponentElements = (
  ...
  // 마인드맵 DEFAULT_CONFIGS는 세로 기준 — 가로 캔버스면 해당 방향으로 노드를 재생성
