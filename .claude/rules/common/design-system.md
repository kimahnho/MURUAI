...
9. **커스텀 색상 유틸리티 수동 등록 필수** — primary/black/status(error/warning/success) 색상은 Tailwind v4 자동 생성이 아닌 `global.css`의 `@layer utilities`에 수동 등록. 새 색상 변형(`hover:bg-*`, `active:bg-*` 등)이 필요하면 `global.css`에 추가해야 동작함
