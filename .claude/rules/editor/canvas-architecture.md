...
## DesignPaper transition 규칙

- DesignPaper 컨테이너에 `transition-all` 금지 — `width`/`height` 변경 시 로고/요소가 슬라이드되는 시각적 결함 발생
- 전이가 필요한 경우 개별 속성만 지정 (`transition-shadow`, `transition-colors` 등)

## CanvasStage 초기 렌더 규칙

- CanvasStage의 `inline-flex center` 래퍼 때문에, canvas 크기가 0이면 페이퍼가 화면 중앙에 배치됨
- `useCanvasZoom`의 `useEffect` 실행 전에도 canvas가 올바른 크기를 갖도록, `canvasWidth`/`canvasHeight`를 인라인 스타일로 미리 설정 필수
- 이 값은 `useCanvasZoom`에서 동기적으로 계산하여 반환
