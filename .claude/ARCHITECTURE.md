...
- **업로드 경로**: `muru_uploads/{userId}` (사용자), `muru_emotion_scene/{userId}` (AI 장면), `muru-templates/admin/{template-name}/` (템플릿 배경), `muru_user_ai_gen/{userId}` (Image-Gen AI), `muru-landing/` (랜딩 갤러리), `muru-saved-characters/{userId}` (스토리북 저장 캐릭터), `muru_storybook_gen/{userId}` (스토리북 삽화)
- **지원 포맷**: JPEG, PNG, SVG
